            ...
            {actividadReciente.length === 0 ? (
              ...
              <div className="space-y-3">
                {actividadReciente.map((item, i) => (
                  <div key={i} className={`flex items-center gap-4 rounded-[16px] border border-slate-100 p-4 shadow-sm ${i%2===0?'bg-white':'bg-[#f8fafd]'}`}>
                    <div className={`w-1 h-10 rounded-full flex-shrink-0 ${item.status==='realizada'?'bg-[#2f7a31]':'bg-[#b88d00]'}`}/>
                    <div className={`flex h-9 w-9 items-center justify-center rounded-full text-white font-bold text-sm flex-shrink-0 ${item.status==='realizada'?'bg-[#2f7a31]':'bg-[#b88d00]'}`}>
